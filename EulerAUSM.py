...
Nx = 500  # Number of space discretizations
...
# Time Discretization
dt = 1e-7  # Time step
Nt = 100  # Number of time iterations
...
for i in range(len(x) - 1):
    ...
    solution[Rho_idx] = quad(rho, x[i], x[i + 1])[0] / dx
    solution[Momentum_idx] = quad(momentum, x[i], x[i + 1])[0] / dx
    solution[Energy_idx] = quad(rhoEt, x[i], x[i + 1])[0] / dx

    for j in range(mix.nSpecies()):
        Rhos_idx = DOF_HANDLER(i, j, mix.nSpecies(), mix.nEnergyEqns(), 'Specie')
        dens = lambda xx: solution[i * (2 + mix.nEnergyEqns() + mix.nSpecies())] * Ys[j]
        solution[Rhos_idx] = quad(dens, x[i], x[i + 1])[0] / dx
...
def RHS(t, solution):
    ...
    dFprev = np.zeros(2 + mix.nEnergyEqns() + mix.nSpecies())
    dFnext = np.zeros(2 + mix.nEnergyEqns() + mix.nSpecies())
    ws_integrated = np.zeros((2 + mix.nEnergyEqns() + mix.nSpecies()) * len(meshPoints))
    dF = np.zeros((2 + mix.nEnergyEqns() + mix.nSpecies()) * len(meshPoints))
    ...
    for i in range(len(meshPoints)):
        ...
        qPrev = solution[iPrev * nVar:(iPrev + 1) * nVar]
        q = solution[i * nVar:(i + 1) * nVar]
        qNext = solution[iNext * nVar:(iNext + 1) * nVar]
        ...
        dF[i*nVar : (i+1)*nVar] = dFnext - dFprev

        # ----------------------------- Production rates ------------------------------------------------------------- #
        rhos = q[0]*q[2 + mix.nEnergyEqns():-1]
        rhoe = q[2]
        mix.setState(rhos, rhoe, 0)
        ws = mix.netProductionRates()
        ws_integrated[i*nVar + 2+mix.nEnergyEqns():(i+1)*nVar] = ws

    RHS = ws_integrated - dF/dx
    return RHS
...
sol = solve_ivp(RHS, t, solution, t_eval=np.linspace(x[0], x[1], Nt), method="RK45")
...
plt.figure(1)
plt.plot(meshPoints, sol.y[0::nVar, plotidx])
plt.title("Density")

plt.figure(2)
plt.plot(meshPoints, sol.y[1::nVar, plotidx])
plt.title("Momentum")

plt.figure(3)
plt.plot(meshPoints, sol.y[2::nVar, plotidx])
plt.title("Energy")

plt.figure(4)
plt.plot(meshPoints, sol.y[2 + mix.nEnergyEqns() + 3::nVar, plotidx])
plt.title("Xi")
