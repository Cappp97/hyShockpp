...
air = mpp.Mixture("air_11")
air.equilibrate(T_inf, P_inf)
c_inf = air.equilibriumSoundSpeed()
u_inf = M_inf * air.equilibriumSoundSpeed()  # equilibrium speed
gamma = air.mixtureEquilibriumGamma()
h_inf = air.mixtureHMass()
...
def rk_jump(state):
    global u_inf, rho_inf, h_inf

    air2 = mpp.Mixture("air_11")
    u, T, P = state

    if P < 0 or T<0 or u<0:
        return [1e+16,1e+16,1e+16]

    air2.equilibrate(T, P)
    rho = air2.density()
    h = air2.mixtureHMass()
